...
export const VARIATION_SCHEMA = yup.object().shape({
  size: yup.string(),
  qty: yup.number().typeError("Amount must be a number").required("Please provide an item price.").min(0, "At least one is required"),
  price: yup.number().typeError("Amount must be a number").required("Please provide an item price.").min(0, "Price can not be zero."),
});
